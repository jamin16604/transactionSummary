...
def test_summary_invalid_userid_range_data(client):
    response = client.get("/summary/9999", params={
...
def test_summary_invalid_date_range(client):
    response = client.get("/summary/100", params={
...
def test_summary_invalid_userid(client):
    response = client.get("/summary/0", params={
        "start_date": "2023-01-01T00:00:00",
        "end_date": "2023-12-31T23:59:59"
    })
    assert response.status_code == 422
    response = client.get("/summary/a", params={
        "start_date": "2023-01-01T00:00:00",
        "end_date": "2023-12-31T23:59:59"
    })
    assert response.status_code == 422
...
def test_summary_edge_case_dates(client):
    client.post("/upload/", files={"file": ("edge_case.csv",
