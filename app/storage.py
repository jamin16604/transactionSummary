...
def load_summary(user_id: int, start_date: datetime, end_date: datetime) -> SummaryResponse:
    """
    ...
    try:
        ...
        if result is None or any(value is None for value in result):
            raise HTTPException(status_code=404,detail="No data found for the given user_id and date range.")
